...
Publication plotter for the ClonoDynamics support-conditioned synthetic
positive-control validation.

The script reads the finalized summary tables generated by
`04_summarize_temporal_recovery.py`. It does not generate synthetic repertoires,
rerun Steps 2/4/5/11/13, refit temporal slopes, or rerun bootstrap draws.

Required inputs in --summary-dir
--------------------------------
...
Default publication outputs
---------------------------
    Figure_validation_A_temporal_profiles.[pdf/png]
    Figure_validation_B_incremental_recovery.[pdf/png]
...
The optional absolute oracle-versus-recovered panel is enabled only with
`--include-supplementary`. PDF and PNG are enabled by default; SVG is optional.
All plotting aesthetics and scientific data-selection rules are defined below
and are preserved by the figure orchestrator.

Typical use
-----------
    python3 plot_positive_controls.py \
        --summary-dir dataset_longitudinal_results/positive_controls/summary \
        --outdir figures/03_validation/positive_controls
"""
