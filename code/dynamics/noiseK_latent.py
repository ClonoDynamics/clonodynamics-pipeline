...
Latent-frequency inference engine for the ClonoDynamics technical-replicate
noise model.
...
For every subject-timepoint pair, Step 2 supplies paired technical-replicate
clonotype counts and sequencing depths. This module fits a shared
Negative-Binomial observation model with a power-law-shaped latent-frequency
prior and reconstructs a clonotype-specific posterior distribution over latent
relative frequency.

The module provides:

    - pair-specific estimation of the prior exponent gamma and
      Negative-Binomial overdispersion parameter kappa;

    - joint latent-frequency posteriors from both technical replicates;

    - single-replicate posterior summaries for technical diagnostics and
      replicate-decoupled downstream analyses;

    - exact discrete-grid Bhattacharyya overlap between the two
      single-replicate posteriors;

    - posterior-predictive replicate and paired-state detectability;

    - optional storage of the full joint posterior matrix for downstream
      transition-posterior propagation;

    - Monte-Carlo propagation of independent endpoint latent posteriors into
      finite-time log-frequency displacement summaries.

The module does NOT impose any longitudinal dynamical model.
...
Here:
    ...
    kappa
        common Negative-Binomial size/overdispersion parameter.
...
paired observation for latent-frequency inference.
...
Power-law prior: exact implementation
-------------------------------------
For grid point f_j, the current implementation assigns normalized DISCRETE
prior mass:
...
The code treats `f_j^(-gamma)` as probability MASS on the log-spaced grid.
It does not multiply by grid-cell widths as a quadrature approximation to a
continuous density `p(f) df`.

Therefore manuscript wording should describe the implementation as a
power-law-shaped discrete prior on a log-spaced latent-frequency grid unless
the numerical integration scheme is changed explicitly.


Joint posterior
---------------
For one clonotype with count pair:

    (c_1, c_2),

the joint latent posterior on the shared frequency grid is:
...
For clonotype i, define its marginal count-pair probability on the discrete
grid:
...
Numerically, both parameters have lower bound:
...
Chunking changes only computation strategy, not the fitted objective.
...
Consensus latent state
----------------------
The JOINT posterior:

    P(f | c_rep1,c_rep2)

is the consensus latent timepoint state used by the main trajectory workflow.

Single-replicate posteriors are retained for diagnostics and for downstream
replicate-decoupled estimands; they do not replace the joint state estimate.
...
The result is numerically clipped to:

    [0,1].

This is the exact coefficient for the two normalized DISCRETE posteriors
constructed by this module.


Replicate displacement diagnostics
----------------------------------
The module provides convenience replicate-difference fields:
...
the mean difference under independent endpoint/posterior interpretation.
...
are differences of marginal summaries, not the median/mode of a formally
...
are conservative endpoint-derived bounds, not calibrated posterior quantiles of
...
Formal downstream displacement posteriors should instead propagate the full
posterior grids.
...
Paired-state detectability
--------------------------
The latent timepoint state is defined as detected when at least one of the two
technical replicates reaches the count threshold.
...
The exact posterior-predictive paired-state dropout probability implemented is:
...
because the two replicate predictive probabilities are coupled through the same
posterior uncertainty in f.
...
The function assumes INDEPENDENT endpoint posterior draws.

It does not construct a longitudinal joint posterior linking the two
timepoints.
...
The module still returns the fitted parameter vector and posterior outputs
constructed from the optimizer result even when the optimizer's success flag is
False; callers should therefore inspect `success` explicitly.
...
Step 2 is responsible for:
    ...
    defining downstream empirical logP observability summaries.
...
It should not be described as:
    ...
    a response-detection model between timepoints;
...
The main inferential object is the posterior distribution over latent
clonotype frequency for one biological timepoint observed through paired
