#!/usr/bin/env python3
"""
noise_pipeline_full.py
v3 is the english version of v2(italian)

End-to-end pipeline (NO notebook):
1) Fit noise model per pair of technical replicates (subject_time: replicate 1 vs 2)
2) Robust QC on fitted parameters (MAD z-score)
3) Choose the null model (clone/subject/global) + observability cutoff on logP
4) Self-consistent probabilistic denoising with the chosen null (p_emp_low = empirical CDF within pool)
5) Aggregate null parameters (for reporting/downstream)
6) Output "ready for trajectories" (per-clonotype observability over time)

Requirements:
- Python 3.9+
- pandas, numpy
- an importable module "noiseK_nb" exposing: fit_noiseK_nb_powerlaw(...)
- input replicate files in TSV with columns: aaSeqCDR3, readCount
"""
...
def index_pairs(data_dir: Path, pattern: re.Pattern) -> List[Tuple[int, int, Path, Path]]:
    ...
    # Auto-discovery of complete pairs (replicate 1 & 2)
    pairs = []
...
def run_noise_fit(
    ...
    """
    Run the noise fit for each (subject,time) pair and save:
    - results_dir/null_params_by_pair_std.csv
    - perclone_dir/null_per_clone_logP_std{subject_time}.csv

    Notes:
    - Each per-clone CSV contains the model-derived logP plus observed counts/frequencies.
    - Depths (total reads per replicate) are appended as constant columns for auditability.
    """
    try:
    ...
    for subject, time, fp1, fp2 in pairs:
        ...
        try:
            ...
            # Frequencies (useful downstream for dynamics)
            ...
            # Enrich per-clone logP with counts/frequencies from the pair
            ...
            if "aaSeqCDR3" in per_clone.columns:
                ...
                # Fallback: use the first column as key (less safe, but prevents a crash)
            ...
            if extra is not None:
                # If per_clone already contains any of these columns, pandas will create *_x / *_y suffixes.
                # This is expected and can be used to audit consistency between sources.
                per_clone = per_clone.merge(extra, on=key_col, how="left")

            # Add depths as constants (convenient for audit)
            ...
            # Add all fit parameters (if available)
...
def run_qc(
    ...
    # Consider only successful fits
    df_qc = df[df.get("success", False) == True].copy()
    if df_qc.empty:
        # Still save a QC file marking all as failed
        df_fail = df.copy()
    ...
    # Columns expected from the fit (gamma, k, log_likelihood)
    for col in ["gamma", "k", "log_likelihood"]:
        if col not in df_qc.columns:
            raise ValueError(f"QC: missing column '{col}' in {null_params_csv}. Columns: {list(df_qc.columns)}")
...
def load_perclone_with_metadata(perclone_dir: Path, df_qc: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    pair_pat = re.compile(r'(?P<sid>\d+_\d+)')  # extract subject_time from any filename containing \d+_\d+
    for fp in files:
    ...
    perclone = pd.concat(rows, ignore_index=True)
    return perclone
...
def apply_pool_and_cutoff(
    ...
    # cutoffs per pool (kept for reporting/audit)
    cutoffs = (
    ...
    # IMPORTANT:
    # keep old cutoff-based flag ONLY as a diagnostic (do NOT use downstream)
...
def denoise_empirical(perclone: pd.DataFrame, logp_col: str, choice: str) -> pd.DataFrame:
    # Reference distributions ONLY from QC-pass pairs
    clean = perclone[perclone["QC_pass_pair"] == True].copy()
    if clean.empty:
    ...
    # Ensure GLOBAL fallback for cases where some pool_id does not exist
...
def aggregate_null_params(df_qc: pd.DataFrame, choice: str, results_dir: Path) -> Optional[Path]:
    ...
    if params_clean.empty:
        raise ValueError("No QC-pass pairs: cannot aggregate parameters.")

    # Candidate columns often present in the fit
    ...
    if choice == "subject":
        agg = (
            params_clean.groupby("subject")[cols]
    ...
    # choice == clone: parameters are already per-pair
    print("choice=clone: using per-pair parameters already in null_params_by_pair_std.csv")
    return None


def trajectory_outputs(perclone: pd.DataFrame, clone_col: str, choice: str, alpha: float, results_dir: Path) -> Tuple[Path, Path]:
    # Keep same outputs (columns observable + p_emp_low) but now observable is p-value based
    traj = (
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Full pipeline: noise fit -> QC -> null choice -> cutoff -> denoising -> aggregations"
    )
    p.add_argument("--data-dir", type=Path, required=True,
                   help="Folder with replicate files (e.g., repertori_longitudinali) named like '1_1-1' and '1_1-2'")
    ...
    p.add_argument("--alpha", type=float, default=0.01,
                   help="Alpha quantile for the observability cutoff on logP (default 0.01)")
    p.add_argument("--min-qc-pairs-per-subject", type=int, default=3,
                   help="If null=subject, minimum QC-pass pairs required to use the subject pool (otherwise GLOBAL).")

    # NEW: tail selection for p-value definition (does NOT change any upstream fit)
    p.add_argument(
        "--tail",
        choices=["low", "high"],
        default="low",
    ...
    p.add_argument("--skip-fit", action="store_true",
                   help="Skip the fit step and use the existing results-dir/null_params_by_pair_std.csv")
    p.add_argument("--skip-qc", action="store_true",
                   help="Skip QC and use the existing results-dir/QC_noise_model_results_std.csv")
...
def main():
    ...
    if args.skip_qc:
        if not qc_out_csv.exists():
            raise FileNotFoundError(f"--skip-qc requested but missing: {qc_out_csv}")
    ...
    # 4) pool null + cutoff (kept for audit) - does NOT define final observable anymore
    ...
    # 5) empirical denoising consistent with the chosen null
    perclone = denoise_empirical(perclone=perclone, logp_col=logp_col, choice=args.null_choice)

    # -----------------------------
    # FIX: define p_value + observable in a self-consistent way
    ...
    # SAFETY ASSERT: prevent impossible states
    ...
    # 7) output ready for trajectories
    trajectory_outputs(perclone=perclone, clone_col=clone_col, choice=args.null_choice, alpha=args.alpha, results_dir=results_dir)
